...
* `users_restored` / `usersRestored`  (int64): Number of users successfully restored (only present for `restoration_type=users`).
* `users_errored` / `usersErrored`  (int64): Number of users that failed to restore (only present for `restoration_type=users`).
* `users_total` / `usersTotal`  (int64): Total number of users processed (only present for `restoration_type=users`).
* `api_keys_restored` / `apiKeysRestored`  (int64): Number of API keys restored (only present for `restoration_type=users`).
* `public_keys_restored` / `publicKeysRestored`  (int64): Number of public keys restored (only present for `restoration_type=users`).
* `two_factor_authentication_methods_restored` / `twoFactorAuthenticationMethodsRestored`  (int64): Number of two factor authentication methods restored (only present for `restoration_type=users`).
...
* `workspace_id` / `workspaceId`  (int64): Workspace ID for a workspace-scoped restore. `0` means the default site-wide scope.
...
* `workspace_id` (Long): Workspace ID for a workspace-scoped restore. `0` means the default site-wide scope.
