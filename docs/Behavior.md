...
* `disable_parent_folder_behavior` / `disableParentFolderBehavior`  (boolean): If true, the parent folder's behavior will be disabled for this folder.
...
* `sort_by` (Map<String, String>): If set, sort records by the specified field in either `asc` or `desc` direction (e.g. `sort_by[behavior]=desc`). Valid fields are `behavior` and `impacts_ui`.
