...
* `receive_admin_alerts` / `receiveAdminAlerts`  (boolean): Deprecated. Use notify_on_all_site_warnings and granular failure notification preferences instead.
* `notify_on_all_site_warnings` / `notifyOnAllSiteWarnings`  (boolean): Should the user receive site warnings via email?
* `notify_on_all_sso_failures` / `notifyOnAllSsoFailures`  (boolean): Should the user receive sso/scim/ldap configuration/sync failures via email?
* `notify_on_all_user_security_events` / `notifyOnAllUserSecurityEvents`  (boolean): Should the user receive user security events via email?
* `notify_on_all_pending_work_failures` / `notifyOnAllPendingWorkFailures`  (boolean): Should the user receive pending work failures via email?
* `notify_on_all_siem_http_destination_failures` / `notifyOnAllSiemHttpDestinationFailures`  (boolean): Should the user receive siem failures via email?
...
* `desktop_configuration_profile_id` (Long): Desktop Configuration Profile ID assigned directly to this user, if any.
* `default_workspace_id` (Long): Workspace ID the user should land in by default when more than one Workspace is available.
...
parameters.put("default_workspace_id", 1);
...
parameters.put("notify_on_all_site_warnings", true);
parameters.put("notify_on_all_sso_failures", true);
parameters.put("notify_on_all_user_security_events", true);
parameters.put("notify_on_all_pending_work_failures", true);
parameters.put("notify_on_all_siem_http_destination_failures", true);
parameters.put("notify_on_all_sync_failures", true);
parameters.put("notify_on_all_automation_failures", true);
