# Files.Models.SyncRunLiveTransfer
...
* `bytes_copied` / `bytesCopied`  (int64): Bytes transferred so far
* `bytes_total` / `bytesTotal`  (int64): Total bytes of the file being transferred
* `percentage` / `percentage`  (double): Transfer progress from 0.0 to 1.0
