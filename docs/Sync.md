...
* `include_patterns` / `includePatterns`  (array(array)): Array of glob patterns to include
* `exclude_patterns` / `excludePatterns`  (array(array)): Array of glob patterns to exclude
...
* `interval` (Long): Interval in minutes for sync (if scheduled)
