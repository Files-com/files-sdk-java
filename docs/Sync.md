...
* `src_site_id` / `srcSiteId`  (int64): Source site ID if syncing from a child or partner site
* `dest_site_id` / `destSiteId`  (int64): Destination site ID if syncing to a child or partner site
...
* `always_write_trigger_file` / `alwaysWriteTriggerFile`  (boolean): If true, the trigger file will be sent at the end of a successful sync even when no files were transferred.
...
HashMap<String, Object> parameters = new HashMap<>();
parameters.put("delete_empty_folders", true);
...
parameters.put("exclude_patterns", ["example"]);
parameters.put("holiday_region", "us_dc");
parameters.put("include_patterns", ["example"]);
...
parameters.put("always_write_trigger_file", true);
